...
// Helper function to get month labels for heatmap
const getMonthLabels = (contributions: Contribution[]): string[] => {
  const months: string[] = []
  let currentMonth = ''
  ...
  contributions.forEach((cont, index) => {
    ...
    // Add month label at the start of each month (approximately every 30 days)
    if (month !== currentMonth && index % 30 < 7) {
      months.push(month)
      currentMonth = month
    } else if (index === 0) {
      months.push(month)
